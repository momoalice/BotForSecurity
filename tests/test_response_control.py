import unittest
from BotForSecurity import response_control
import os
...
class TestBot(unittest.TestCase):
	...
	def test_specific(self):
		...
		actual_response = BotForSecurity.postSpecific("Specific 123".split(" "), "Search")[0]
		self.assertEqual(expected_response,actual_response)

	def test_Search(self):
		expected_error_output = "Invalid input. Search command takes in one argument. Please reenter."
		self.assertTrue(BotForSecurity.postSearch(["Search"], "")[0] == expected_error_output and BotForSecurity.postSearch(["Search"], "")[1].lower() == "")

	def test_Sample(self):
		expected_error_output = "Invalid input. Sample command takes in one argument. Please reenter."
		self.assertTrue(BotForSecurity.postSample(["Sample"], "Counts")[0] == expected_error_output and BotForSecurity.postSample(["Sample"], "Counts")[1].lower() == "counts")

	def test_Severity(self):
		expected_error_output = "Invalid input. Severity command takes in one argument. Please reenter."
		self.assertTrue(BotForSecurity.postSeverity(["Severity"], "Counts")[0] == expected_error_output and BotForSecurity.postSeverity(["Severity"], "Counts")[1].lower() == "counts")
